...
class axi4_mem : public axi4_slave<A_WIDTH,D_WIDTH,D_WIDTH2,ID_WIDTH>  {
    public:
        axi4_mem(size_t size_bytes) {
            // printf("!!!!!!!!!!!!!!!!!!!!!!size_bytes is %llu\n",size_bytes);
            if (size_bytes % (D_WIDTH2 / 8)) size_bytes += 8 - (size_bytes % (D_WIDTH2 / 8));
            mem = new unsigned char[size_bytes];
            mem_size = size_bytes;
            // printf("mem_size is %llu\n",mem_size);
