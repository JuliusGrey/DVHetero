...
class mul(val muxW : Int) extends Module{
  ...
  if (signalMul) {
    ...
    val mulMux = resultState
    val resMux = Mux(io.block,resultState, idleState)
    stateReg := MuxLookup(
      stateReg,
      idleState,
      Array(
    ...
    val multiplicandReg = RegEnable(io.multiplicand,0.U,isIdle && io.mul_valid)
    val multiplierReg = RegEnable(io.multiplier, 0.U, isIdle && io.mul_valid)
    val res = multiplicandReg * multiplierReg

    val resReg = RegEnable(res,0.U,isMul)
    io.result_low :=  resReg(muxW - 1, 0)
    io.result_hi := DontCare
    io.out_valid := isRes


    //    io.out_valid := io.mul_valid
//    val res = io.multiplicand * io.multiplier
//    io.result_low := res(muxW - 1, 0)
//    io.result_hi := DontCare
  } else {
    val idleState :: mulState :: resultState :: Nil = Enum(3)
    val stateReg = RegInit(idleState)
